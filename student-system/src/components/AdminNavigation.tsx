...
import { GraduationCap, LogOut, LayoutDashboard, BookOpen, Users, Trophy, ChevronLeft, ChevronRight, Clock, UserCircle2 } from "lucide-react";
...
export const AdminNavigation = ({ activeTab, setActiveTab, setIsExpanded }: AdminNavigationProps) => {
  ...
  return (
    <aside className={cn(
      ...
      <div className="flex items-center justify-center h-16 border-b border-primary-hover/20 relative">
        <Link to="/admin/dashboard" className={cn("flex items-center space-x-2", isExpandedState ? "pr-4" : "")}>
      ...
      {isExpandedState && (
        <div className="flex flex-col items-center p-4 border-b border-primary-hover/20">
          ...
          <p className="text-lg font-semibold text-white whitespace-nowrap">Welcome, Admin</p>
      ...
      <div className="border-t border-primary-hover/20 p-4">
        {isExpandedState && (
          <div className="flex items-center space-x-2 mb-2">
            ...
            <p className="text-base text-primary-foreground/80 whitespace-nowrap">Welcome, {user?.name || 'Admin'}</p>
          </div>
        )}
        <Button 
          variant="ghost" 
